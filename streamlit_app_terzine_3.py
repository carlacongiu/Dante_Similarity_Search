...
def find_similar(query, model, limit=10, cantiche=[], canti=[]):
    ...
    filter_conditions = []
    if cantiche:
        filter_conditions.append(Filter.by_property("cantica").contains_any(cantiche))
    if canti:
        filter_conditions.append(Filter.by_property("canto").contains_any(canti))

    combined_filter = None
    if filter_conditions:
        combined_filter = filter_conditions[0]
        for additional_filter in filter_conditions[1:]:
            combined_filter = combined_filter.and_(additional_filter)
